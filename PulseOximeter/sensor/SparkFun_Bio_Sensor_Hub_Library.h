...
#define SPARKFUN_BIO_SENSOR_HUB_DISABLE 0x00
#define SPARKFUN_BIO_SENSOR_HUB_ENABLE 0x01
#define SPARKFUN_BIO_SENSOR_HUB_MODE_ONE 0x01
#define SPARKFUN_BIO_SENSOR_HUB_MODE_TWO 0x02
#define SPARKFUN_BIO_SENSOR_HUB_APP_MODE 0x00
#define SPARKFUN_BIO_SENSOR_HUB_BOOTLOADER_MODE 0x08

typedef struct bioData_s
{
    uint32_t irLed;
    uint32_t redLed;
...
// Read Bytes under Family Byte: SET_DEVICE_MODE (0x01) and Index
// Byte: 0x00.
enum DEVICE_MODE_READ_BYTES
{
    DEVICE_MODE_APPLICATION = 0x00,
    DEVICE_MODE_RESET = 0x02,
    DEVICE_MODE_BOOTLOADER = 0x08
};
...
// Functions ------------
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readSensorHubStatus(uint8_t *);
    ...
    // well as reset.
    // INCOMPLETE
    uint8_t SparkFun_Bio_Sensor_Hub_setOperatingMode(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_configBpm(uint8_t);
    ...
    // and Green.
    uint8_t SparkFun_Bio_Sensor_Hub_configSensor(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_configSensorBpm(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readBpm(bioData_t *);
    ...
    // into the whrmFifo and returned.
    uint8_t SparkFun_Bio_Sensor_Hub_readSensor(bioData_t *);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readSensorBpm(bioData_t *);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_setPulseWidth(uint16_t);
    ...
    uint16_t SparkFun_Bio_Sensor_Hub_readPulseWidth(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_setSampleRate(uint16_t);
    ...
    uint16_t SparkFun_Bio_Sensor_Hub_readSampleRate(void);
    ...
    // 32.25pA - 8192nA
    // 62.5pA  - 16384nA
    uint8_t SparkFun_Bio_Sensor_Hub_setAdcRange(uint16_t);

    // MAX30101 Register: CONFIGURATION_REGISTER (0x0A), bits [6:5]
    // This function returns the set ADC range of the MAX30101 sensor.
    uint16_t SparkFun_Bio_Sensor_Hub_readAdcRange(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_getMcuType(void);
    ...
    int32_t SparkFun_Bio_Sensor_Hub_getBootloaderInf(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_max30101Control(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readMAX30101State(void);
    ...
    // (begin).
    uint8_t SparkFun_Bio_Sensor_Hub_setFifoThreshold(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_numSamplesOutFifo(uint8_t *);

    // Family Byte: READ_DATA_OUTPUT (0x12), Index Byte: READ_DATA (0x00), Write
    // Byte: NONE
    // This function returns the data in the FIFO.
    uint8_t SparkFun_Bio_Sensor_Hub_getDataOutFifo(uint8_t *);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_numSamplesExternalSensor(uint8_t *);
    ...
    void SparkFun_Bio_Sensor_Hub_writeRegisterMAX30101(uint8_t, uint8_t);
    ...
    // non-successful write.
    void SparkFun_Bio_Sensor_Hub_writeRegisterAccel(uint8_t, uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readRegisterMAX30101(uint8_t);

    // Family Byte: READ_REGISTER (0x41), Index Byte: READ_MAX30101 (0x03), Write Byte:
    // Register Address
    // This function reads the given register address for the MAX30101 Sensor and
    // returns the values at that register.
    uint8_t SparkFun_Bio_Sensor_Hub_readRegisterAccel(uint8_t);
    ...
    // and the number of registers available.
    sensorAttr_t SparkFun_Bio_Sensor_Hub_getAfeAttributesAccelerometer(void);
    ...
    // register value n.
    uint8_t SparkFun_Bio_Sensor_Hub_dumpRegisterAccelerometer(uint8_t, uint8_t regArray[]);
    ...
    // 100 percent.
    uint8_t SparkFun_Bio_Sensor_Hub_setAlgoRange(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_setAlgoStepSize(uint8_t);

    // Family Byte: CHANGE_ALGORITHM_CONFIG (0x50), Index Byte:
    // SET_SENSITIVITY (0x00), Write Byte: AGC_SENSITIVITY_ID (0x02)
    // This function changes the sensitivity of the AGC algorithm.
    uint8_t SparkFun_Bio_Sensor_Hub_setAlgoSensitivity(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_setAlgoSamples(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_setMaximFastCoef(int32_t, int32_t, int32_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readAlgoRange(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readAlgoStepSize(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_readAlgoSensitivity(void);

    // Family Byte: READ_ALGORITHM_CONFIG (0x51), Index Byte:
    // READ_AGC_NUM_SAMPLES (0x00), Write Byte: READ_AGC_NUM_SAMPlES_ID (0x03)
    // This function changes the number of samples that are averaged.
    // It takes a paramater of zero to 255.
    uint8_t SparkFun_Bio_Sensor_Hub_readAlgoSamples(void);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_agcAlgoControl(uint8_t);

    // Family Byte: ENABLE_ALGORITHM (0x52), Index Byte:
    // ENABLE_WHRM_ALGO (0x02)
    // This function enables (one) or disables (zero) the wrist heart rate monitor
    // algorithm.
    uint8_t SparkFun_Bio_Sensor_Hub_maximFastAlgoControl(uint8_t);
    ...
    bool SparkFun_Bio_Sensor_Hub_setNumPages(uint8_t);
    ...
    version_t SparkFun_Bio_Sensor_Hub_readBootloaderVers(void);
    ...
    version_t SparkFun_Bio_Sensor_Hub_readAlgorithmVersion(void);

    // Family Byte: CHANGE_ALGORITHM_CONFIG (0x50), Index Byte: BPT_CONFIG (0x04),
    // Write Byte: BPT_MEDICATION (0x00)
    uint8_t SparkFun_Bio_Sensor_Hub_isPatientBPMedication_8(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_writeDiastolicVals(uint8_t, uint8_t, uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_writeSystolicVals(uint8_t, uint8_t, uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_isPatientResting_8(uint8_t);
    ...
    uint8_t SparkFun_Bio_Sensor_Hub_writeSP02AlgoCoef(int32_t, int32_t, int32_t);
